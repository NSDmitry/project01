...
- `POST /api/auth/login` - вход по номеру телефона и паролю; в ответе только `session_id`. При подборе пароля номер временно блокируется (`429`) - см. [Аутентификация и сессии](auth.md)
