...
целостности. Добавляем FK: book_clubs.owner_id -> users, threads.author_id
-> users, threads.club_id -> book_clubs (ON DELETE CASCADE, чтобы удаление
...
def upgrade() -> None:
    ...
    discussion_fks = {fk['name'] for fk in inspector.get_foreign_keys('threads')}
    if 'fk_discussions_club_id_book_clubs' not in discussion_fks:
        op.create_foreign_key(
            'fk_discussions_club_id_book_clubs', 'threads', 'book_clubs', ['club_id'], ['id'], ondelete='CASCADE'
        )
    if 'fk_discussions_author_id_users' not in discussion_fks:
        op.create_foreign_key('fk_discussions_author_id_users', 'threads', 'users', ['author_id'], ['id'])
...
def downgrade() -> None:
    ...
    if 'fk_discussions_author_id_users' in discussion_fks:
        op.drop_constraint('fk_discussions_author_id_users', 'threads', type_='foreignkey')
    if 'fk_discussions_club_id_book_clubs' in discussion_fks:
        op.drop_constraint('fk_discussions_club_id_book_clubs', 'threads', type_='foreignkey')
