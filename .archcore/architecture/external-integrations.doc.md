...
- S3-совместимое хранилище - via `boto3`, обложки клубов и аватары пользователей (@app/core/media.py). MinIO в docker-compose, Object Storage в проде. Пустой S3_ENDPOINT_URL = запись в локальный каталог, тем же приёмом, что и пустой RABBITMQ_URL (тесты, запуск без бакета). Бакет приватный: наружу картинки раздаёт приложение по `GET /media/<ключ>`, ссылок на хранилище клиент не получает.
