...
7. WHEN операция успешна, роутер MUST вернуть данные в конверте `ResponseModel` с `errors = []`.
...
## Conformance
Реализация конформна, когда выполняет поведения 1-7 и правила отказов 1-6; фактические коды задекларированы в `responses` каждого роута и проверяются тестами tests/sso_tests/ и tests/users/.