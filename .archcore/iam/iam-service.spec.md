...
- Инвариант: каскад удаления аккаунта целиком идёт в транзакции запроса - репозитории соседних доменов собираются на той же сессии `Depends(get_db)`, что и весь запрос, поэтому упавший шаг откатывает весь каскад. За кодом только то, чего не знает БД: развилки по флагам запроса и денормализованные счётчики.
