...
7. WHEN собирается ответ, сервис MUST подтягивать авторов батчем через `UsersPort.get_summaries_by_ids` (relationship на пользователя в модели нет) и заполнять `likes_count`/`is_liked` явно.
