...
4. Каскад удаления пользователя переведён на прямые вызовы в транзакции запроса (задача #78 эпика #75); событий `USER_DELETED`/`CLUBS_DELETED` в `app.core.events` больше нет. `threads_count` тем же способом правится прямым вызовом `change_threads_count` (#77). Шина `app.core.events` физически остаётся до задачи #79; новых подписок на события MUST NOT появляться.
