...
4. Переходное: каскады удаления пользователя пока идут через события `app.core.events` (USER_DELETED, CLUBS_DELETED); эпик #75 (задачи #78-#79) переводит их на прямые вызовы в транзакции запроса и сносит шину. `threads_count` уже правится прямым вызовом `change_threads_count` в транзакции запроса (#77). Новых подписок на события MUST NOT появляться.
