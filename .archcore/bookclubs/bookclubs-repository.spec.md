...
6. `delete_book_club`, `set_genres`, `update_book_club` и `update_cover` MUST принимать уже загруженный объект клуба и только мутировать его. Клуб загружает сервис, он же проверяет владельца через `require_permission` до вызова репозитория. Авторизации (и импорта `require_permission`) в репозитории быть MUST NOT: HTTP и авторизация - в роутере/сервисе, в репозитории - только SQL.
...
31. `update_cover` MUST записывать в `book_clubs.cover_key` ключ файла в хранилище картинок; сервис загружает клуб сам, чтобы проверить права и узнать ключ прежней обложки до обработки файла. Сам файл репозиторий не трогает.
