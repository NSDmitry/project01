...
21. Чтение захода MUST идти запросом, а не выдачей объекта из identity map: книга подтягивается загрузчиком при выполнении SELECT, а уже загруженный объект вернулся бы без неё.
...
25. WHEN участник выходит из клуба, исключается или удаляется пользователь, репозиторий MUST удалить его строки прогресса: иначе доля участников в графике считается от числа участников, а числитель включает ушедших.
...
## Conformance
Реализация конформна, когда выполняет поведения 1-25, держит инварианты уникальности, единственного источника владения, единственного действующего приглашения, генерируемого столбца, счётчика участников и единственного незакрытого захода и правила отказов 1-13. Проверяется тестами tests/bookclubs/ и tests/readings/.
