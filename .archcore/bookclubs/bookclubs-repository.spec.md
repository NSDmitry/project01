...
## Surface
- CRUD: `create_book_club`, `get_book_clubs`, `get_book_club`, `delete_book_club`.
...
3. `delete_book_club` и `set_genres` MUST проверять владельца через `require_permission` до изменения.
4. `set_genres` MUST заменять набор жанров целиком (delete + insert).
5. WHEN приходит USER_DELETED с `delete_clubs=true`, `handle_user_deleted` MUST удалить клубы владельца и вернуть их id (для CLUBS_DELETED); при `false` - занулить `owner_id`; членство пользователя MUST удаляться всегда.
6. `change_threads_count` MUST менять счётчик атомарным UPDATE с `GREATEST(threads_count + delta, 0)` - защита от ухода в минус при дублях событий (at-least-once доставка).
7. Репозиторий MUST завершать записи `flush`, а не `commit` - транзакцию держит session dependency.
...
## Failure Behavior
1. IF имя клуба занято, THEN `create_book_club` MUST откатить и выбросить Conflict.
...
## Conformance
Реализация конформна, когда выполняет поведения 1-7, держит инварианты уникальности и правила отказов 1-4. Проверяется тестами tests/bookclubs/.