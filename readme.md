...
- Публичный профиль пользователя (`GET /api/users/public`) отдаёт только `id` и `name` - без `phone_number`, `session_id` и других приватных полей. Полный профиль (с номером телефона) виден только самому пользователю через `GET /api/users/current`.
