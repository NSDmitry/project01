...
from app.schemas.public_user_schema import OwnUserResponseModel, UpdateUserRequestModel, UserSummaryModel
...
class UserService:
    ...
    async def get_user_by_id(self, user_id: int) -> UserSummaryModel:
        db_user: DBUser = await self.user_repository.get_user_by_id(user_id)

        if db_user is None:
            raise NotFound(errors=["Пользователь с таким id не найден"])

        return ResponseModel.ok(UserSummaryModel.model_validate(db_user))

    async def update_user_info(self, user: DBUser, model: UpdateUserRequestModel) -> OwnUserResponseModel:
        ...
        return ResponseModel.ok(OwnUserResponseModel.model_validate(updated_user))
