...
class DiscussionService:
    ...
    def create_discussion(self, user: DBUser, model: DiscussionCreateRequestModel) -> ResponseModel[DisscussionResponseModel]:
        """
        ...
        db_disscussion = self.discussion_repository.create_discussion(user.id, model)

        return ResponseModel.success_response(DisscussionResponseModel(**db_disscussion.to_dict()))

    def delete_discussion(self, user: DBUser, discussion_id: int) -> ResponseModel:
        """
        Удаление обсуждения.
        :param user:
        :param discussion_id:
